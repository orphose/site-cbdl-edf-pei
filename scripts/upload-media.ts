 ...
 * Usage: npx ts-node scripts/upload-media.ts
 */
...
console.assert(SUPABASE_URL, 'SUPABASE_URL est requis');
console.assert(SUPABASE_ANON_KEY, 'SUPABASE_ANON_KEY est requis');
...
async function uploadFile(localPath: string, remotePath: string): Promise<boolean> {
  console.assert(localPath, 'localPath requis');
  console.assert(remotePath, 'remotePath requis');
