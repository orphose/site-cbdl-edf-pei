...
export default function AdminPage() {
  ...
  // État de protection contre la perte de données
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);

  // Ref pour détecter la première population du formulaire
  const formInitialized = useRef(false);

  // État de l'éditeur Markdown
  ...
  // Détecter les modifications de formulaire (skip initial population)
  useEffect(() => {
    if (viewMode === "list") {
      formInitialized.current = false;
      return;
    }
    // Skip the first change (form population)
    if (!formInitialized.current) {
      formInitialized.current = true;
      return;
    }
    setHasUnsavedChanges(true);
  }, [newsForm, partnershipForm, viewMode]);
  ...
  const saveNews = async () => {
    ...
    try {
      ...
      // Vérifier l'unicité du slug (sauf en édition du même article)
      if (!editingNews || editingNews.slug !== finalSlug) {
        const { data: existing } = await supabase
          ...
          .eq("slug", finalSlug)
          .limit(1);
        if (existing && existing.length > 0 && existing[0].id !== editingNews?.id) {
      ...
      // Only set published_at on create or when toggling to published for the first time
      if (!editingNews) {
        newsData.published_at = newsForm.is_published ? new Date().toISOString() : null;
  ...
  // Supprimer actualité (ouvre la confirmation)
  const deleteNews = (id: string) => {
    setDeleteConfirm({ open: true, type: "news", id, name: news.find(n => n.id === id)?.title || "cet élément" });
  };
  ...
  // Supprimer partenariat (ouvre la confirmation)
  const deletePartnership = (id: string) => {
    setDeleteConfirm({ open: true, type: "partnership", id, name: partnerships.find(p => p.id === id)?.name || "cet élément" });
  };
                ...
                <CardBody className="p-0">
                  {news.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <h3 className="text-lg font-bold text-gray-800 mb-2">Publiez votre première actualité</h3>
                      <p className="text-gray-500 text-sm mb-6 max-w-md mx-auto">
                        Les actualités apparaîtront sur la page d&apos;accueil et la section Actualités du site.
                      </p>
                      <Button onPress={openCreateNews} className="bg-edf-blue text-white" radius="none">
                        Créer une actualité
                      </Button>
                    </div>
                  ) : (
              ...
              >
                <Card className="border border-gray-100 shadow-sm">
                  ...
                  <CardBody className="p-6">
                    ...
                    {(viewMode === "create" || viewMode === "edit") && activeSection === "news" && (
                      <div className="flex gap-4 mb-6 text-sm">
                        <a href="#section-contenu" className="text-edf-blue hover:underline">Contenu</a>
                        <a href="#section-medias" className="text-edf-blue hover:underline">Médias</a>
                        <a href="#section-publication" className="text-edf-blue hover:underline">Publication</a>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="lg:col-span-2 space-y-6">
                        <div id="section-contenu" className="border-b border-gray-200 pb-2 mb-6">
                        ...
                        <div>
                          <label htmlFor="news-slug" className="block text-sm font-medium text-gray-700 mb-2">
                            Slug (URL)
                          </label>
                          <Input
                            id="news-slug"
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) => { if (e.target.value.length <= 300) setNewsForm({ ...newsForm, excerpt: e.target.value }); }}
                            rows={3}
                            maxLength={300}
                          ...
                          />
                          <p className={`text-xs mt-1 ${newsForm.excerpt.length > 260 ? "text-orange-500" : "text-gray-400"}`}>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 mb-2 p-2 bg-gray-100 rounded-t-xl border border-gray-200 border-b-0">
                            <button type="button" onClick={() => insertMarkdown("**", "**")} className="px-2 py-1 text-xs font-bold text-gray-700 hover:bg-gray-200 rounded" title="Gras">G</button>
                            <button type="button" onClick={() => insertMarkdown("*", "*")} className="px-2 py-1 text-xs italic text-gray-700 hover:bg-gray-200 rounded" title="Italique">I</button>
                            <button type="button" onClick={() => insertMarkdown("## ")} className="px-2 py-1 text-xs font-bold text-gray-700 hover:bg-gray-200 rounded" title="Titre">H2</button>
                            <button type="button" onClick={() => insertMarkdown("- ")} className="px-2 py-1 text-xs text-gray-700 hover:bg-gray-200 rounded" title="Liste">Liste</button>
                            <button type="button" onClick={() => insertMarkdown("[", "](url)")} className="px-2 py-1 text-xs text-gray-700 hover:bg-gray-200 rounded" title="Lien">Lien</button>
                            <div className="flex-1" />
                            <button
                              type="button"
                              onClick={() => setContentPreview(!contentPreview)}
                              className={`px-3 py-1 text-xs rounded ${contentPreview ? "bg-edf-blue text-white" : "text-gray-700 hover:bg-gray-200"}`}
                            >
                          ...
                          {contentPreview ? (
                            <div className="w-full px-4 py-3 bg-white border border-gray-200 rounded-b-xl text-sm min-h-[288px] prose prose-sm max-w-none">
                              {newsForm.content ? (
                                <ReactMarkdown>{newsForm.content}</ReactMarkdown>
                              ) : (
                      ...
                      <div className="space-y-6">
                        <div id="section-medias" className="border-b border-gray-200 pb-2 mb-6 mt-8">
                        ...
                        <div id="section-publication" className="border-b border-gray-200 pb-2 mb-6 mt-8">
                          <h3 className="text-lg font-semibold text-gray-800">Publication & Partage</h3>
                        </div>
                        ...
                        <div
                          ...
                          onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); setNewsForm({ ...newsForm, is_published: !newsForm.is_published }); } }}
                        >
                        ...
                        <div className="border border-[#0A66C2]/20 rounded-xl overflow-hidden">
                          <div
                            ...
                            onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); setLinkedInShare({ ...linkedInShare, enabled: !linkedInShare.enabled }); } }}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                ...
                <CardBody className="p-0">
                  {partnerships.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <h3 className="text-lg font-bold text-gray-800 mb-2">Ajoutez votre premier partenaire</h3>
                      <p className="text-gray-500 text-sm mb-6 max-w-md mx-auto">
                        Les partenariats apparaîtront sur la page dédiée du site.
                      </p>
                      <Button onPress={openCreatePartnership} className="bg-edf-blue text-white" radius="none">
                  ...
                  <CardBody className="p-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="lg:col-span-2 space-y-6">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <label htmlFor="partnership-name" className="block text-sm font-medium text-gray-700">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <label htmlFor="partnership-description" className="block text-sm font-medium text-gray-700">
                              Description
                            </label>
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 hover:border-edf-blue/50 transition-colors">
                            {partnershipForm.logo_url ? (
                              ...
                              <div
                                className="flex flex-col items-center justify-center py-8 cursor-pointer"
                                onClick={() => partnershipFileInputRef.current?.click()}
                              >
                                <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center mb-3">
                        ...
                        <div
                          ...
                          onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); setPartnershipForm({ ...partnershipForm, is_active: !partnershipForm.is_active }); } }}
                        >
                          <div className="flex items-center justify-between">
                            <div>
              ...
              <button
                onClick={async () => {
                  if (deleteConfirm.type === "news") {
                    await supabase.from("news").delete().eq("id", deleteConfirm.id);
                  } else {
                    await supabase.from("partnerships").delete().eq("id", deleteConfirm.id);
                  }
